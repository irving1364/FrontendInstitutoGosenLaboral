(function() {
	...
	angular
		...
		.factory('addEvaluacionService', addEvaluacionService);

	function addEvaluacionService($coinexHttp, $q, newAddressResponseFake) {
	...
	Handlers.prototype = {
		...
		action: function(command, success, failed) {
			var data = {	
				...
				nombre: 			command.asunto,
				descripcion: 		command.descripcion,
				fecha: 				command.fecha2,
				hora_fin: 			command.hora_fin2,
				hora_ini: 			command.hora_ini2,
				cod_tipo_evaluacion:command.tipo_eva,
				trimestre: 			command.trimestre,			
				
			};
			return this.coinexHttp.http.post(localStorage.getItem("apiUrl") +'evaluacion/addEvaluacion', data);
		},
