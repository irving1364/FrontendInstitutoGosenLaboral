(function() {
	...
	angular
		...
		.factory('asignarConsejeroCursoService', asignarConsejeroCursoService);

	function asignarConsejeroCursoService($coinexHttp, $q, newAddressResponseFake) {
	...
	Handlers.prototype = {
		...
		action: function(command, success, failed) {
			...
			var data = {	
				...
				cod_curso_detail:command.cod,
				cod_maestro:     command.cod_maestro,
				cod_anio:        localStorage.getItem('cod_anio')
			};
			return this.coinexHttp.http.post(localStorage.getItem("apiUrl") +'tabla/asignarConsejeroCurso', data);
		},
