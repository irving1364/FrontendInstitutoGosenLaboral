(function() {
	...
	angular
		...
		.factory('editarMateriaPrioridadService', editarMateriaPrioridadService);

	function editarMateriaPrioridadService($coinexHttp, $q, newAddressResponseFake) {
	...
	Handlers.prototype = {
		...
		action: function(command, success, failed) {
			...
			var data = {	
				...
				cod_curso_detail:command.cod_curso_detail,
				cod:             command.cod,
				prioridad:   	 command.prioridad,
			};
			return this.coinexHttp.http.post(localStorage.getItem("apiUrl") +'tabla/updatePrioridadMateria', data);
		},
